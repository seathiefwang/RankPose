...
class Pose300wDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.split == 'train':
            ...
            scale = 1.4
        bbox = change_bbox(self.bbox[index], scale=scale, use_forehead=False)
        ...
        if self.split=='train':
            ...
            if random.random() < 0 and abs(label[0])<30 and abs(label[2])<30:
                if random.random() < 0.5:
                    factor = 1
                    label[2] += 90
                    if self.use_bined:
        ...
        if self.use_bined:
            return img, label, int(bined_label[0]), int(bined_label[1]), int(bined_label[2])
        else:
            return img, label
...
if __name__ == '__main__':
    ...
    dataset = Pose300wDataset(base_dir="data", affine_augmenter=affine_augmenter, image_augmenter=image_augmenter,
